...
real(dbl),  private :: ctrl_init_s_sea, ctrl_init_t, ctrl_init_d_liq, ctrl_init_d_vap, &
         ctrl_eps_exit_s, ctrl_eps_exit_t, ctrl_eps_exit_p
real(dbl),  private :: equi_sea_vap_t, equi_sea_vap_s, equi_sea_vap_p
real(dbl),  private :: equi_sea_vap_d_vap, equi_sea_vap_d_liq
...
implicit none
real(dbl) sea_vap_boilingtemperature_si, sa_si, p_si
...
implicit none
real(dbl) sea_vap_brinesalinity_si, t_si, p_si
...
implicit none
real(dbl) sea_vap_cp_seavap_si, ssv_si, t_si, p_si
real(dbl) g_tt
...
implicit none
real(dbl) sea_vap_density_sea_si
...
implicit none
real(dbl) sea_vap_density_seavap_si, ssv_si, t_si, p_si
real(dbl) g_p
...
implicit none
real(dbl) sea_vap_density_vap_si
...
implicit none
real(dbl) sea_vap_enthalpy_evap_si
real(dbl) s, t, p, d
...
implicit none
real(dbl) sea_vap_enthalpy_sea_si
...
implicit none
real(dbl) sea_vap_enthalpy_seavap_si, ssv_si, t_si, p_si
real(dbl) g, g_t
...
implicit none
real(dbl) sea_vap_enthalpy_vap_si
...
implicit none
real(dbl) sea_vap_entropy_sea_si
...
implicit none
real(dbl) sea_vap_entropy_seavap_si, ssv_si, t_si, p_si
real(dbl) g_t
...
implicit none
real(dbl) sea_vap_entropy_vap_si
...
implicit none
real(dbl) sea_vap_expansion_seavap_si, ssv_si, t_si, p_si
real(dbl) g_p, g_tp
...
real(dbl) sea_vap_g_si, ssv_si, t_si, p_si
real(dbl) g, s, w, d
real(dbl) gb, gv
real(dbl) gb_p, gv_p
real(dbl) gb_t, gv_t
real(dbl) gb_pp, gv_pp, gm_pp
real(dbl) gb_tp, gv_tp, gm_tp
real(dbl) gb_tt, gv_tt, gm_tt
real(dbl) gb_s, gb_sp, gb_st
real(dbl) s_p, s_t
...
implicit none
real(dbl) sea_vap_kappa_t_seavap_si, ssv_si, t_si, p_si
real(dbl) g_p, g_pp
...
implicit none
integer drv_t, drv_p
real(dbl) sea_vap_s_si, t_si, p_si
real(dbl) d  !DILUTION COEFF
real(dbl) s  !BRINE SALINITY

real(dbl) gb_p, gv_p, gb_sp
real(dbl) gb_t, gv_t, gb_st
...
implicit none
real(dbl) sea_vap_temperature_si
...
implicit none
real(dbl) sea_vap_vapourpressure_si, sa_si, t_si
...
implicit none
real(dbl) sea_vap_volume_evap_si
real(dbl) s, t, p, d
...
implicit none
integer maxit
real(dbl) set_sea_vap_eq_at_s_p
real(dbl) sa_si, p_si
real(dbl) t, d_liq, d_vap, eps, dvt, dlt, tt
...
implicit none
integer maxit
real(dbl) set_sea_vap_eq_at_s_t
real(dbl) sa_si, t_si
real(dbl) eps
real(dbl) d_liq, d_vap, p_si, dvt, dlt
...
implicit none
integer maxit
real(dbl) set_sea_vap_eq_at_t_p, t_si, p_si
real(dbl) d_liq, d_vap, s_norm
real(dbl) s, eps
...
implicit none
integer it, maxit
real(dbl) sea_vap_iteration_at_s_p, sa_si, p_si, eps, d_liq_si, d_vap_si, t_si
real(dbl) dl, ddl
real(dbl) dv, ddv
real(dbl) t, dt
real(dbl) mus, p
...
implicit none
integer it, maxit
real(dbl) sea_vap_iteration_at_s_t, sa_si, t_si, eps, d_liq_si, d_vap_si, p_si
real(dbl) dl, ddl
real(dbl) dv, ddv
real(dbl) mus, p, p_old
real(dbl) gl, gv
real(dbl) gs_p, gs_sp
...
implicit none
integer it, maxit
real(dbl) sea_vap_iteration_at_t_p, t_si, p_si, eps, s_sea_si
real(dbl) s, ds
real(dbl) mus, sgss
real(dbl) gl, gv
...
implicit none
real(dbl) sea_vap_brinefraction_seavap_si
real(dbl) ssv_si, t_si, p_si
real(dbl) s
...
implicit none
real(dbl) aux_boilingtemperature_si, sa_si, p_si
real(dbl) tt, pt, a1, a2, alpha
real(dbl) p, q
real(dbl) tr, ps
...
implicit none
real(dbl) aux_brinesalinity_si, t_si, p_si
real(dbl) p0, alpha
...
implicit none
real(dbl) aux_vapourpressure_wp80_si, sa_si, t_si
real(dbl) spsu
real(dbl) tabs48_k
real(dbl) psw
...
implicit none
real(dbl) sa_si, t_si, p_si, d_liq_si, d_vap_si
