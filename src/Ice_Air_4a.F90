...
real(dbl) equi_ice_air_a        !DRY AIR MASS FRACTION OF HUMID AIR
real(dbl) equi_ice_air_t        !TEMPERATURE
real(dbl) equi_ice_air_p        !PRESSURE
real(dbl) equi_ice_air_eta      !HUMID AIR ENTROPY
real(dbl) equi_ice_air_d_air    !HUMID AIR DENSITY
...
real(dbl) ice_air_a_from_rh_wmo_si, rh_si, t_si, p_si
real(dbl) a_sat
...
real(dbl) ice_air_a_from_rh_cct_si, rh_si, t_si, p_si
real(dbl) a_sat
...
real(dbl) ice_air_massfraction_air_si, t_si, p_si
...
real(dbl) ice_air_condensationpressure_si, a_si, t_si
...
real(dbl) ice_air_sublimationpressure_si, t_si, p_si
real(dbl) a_sat, xv
...
real(dbl) ice_air_density_air_si
...
real(dbl) ice_air_density_ice_si
...
real(dbl) ice_air_density_vap_si
...
real(dbl) ice_air_enthalpy_subl_si
real(dbl) a, t, dh
real(dbl) hh, hh_a, hi
real(dbl) fh_a, fh_d
...
real(dbl) ice_air_frostpoint_si, a_si, p_si
...
real(dbl) ice_air_icl_si, a_si, t_si, p_si
real(dbl) eta
...
real(dbl) ice_air_ict_si, a_si, t_si, p_si
real(dbl) eta
...
real(dbl) ice_air_rh_cct_from_a_si, a_si, t_si, p_si
real(dbl) a_sat
...
real(dbl) ice_air_rh_wmo_from_a_si, a_si, t_si, p_si
real(dbl) a_sat
...
real(dbl) ice_air_temperature_si
...
real(dbl) set_ice_air_eq_at_a_eta, a_si, eta_si
real(dbl) p_si, t_si, dh, eps
...
real(dbl) set_ice_air_eq_at_a_p, a_si, p_si
real(dbl) t_si, dh, eta, eps
...
real(dbl) set_ice_air_eq_at_a_t, a_si, t_si
real(dbl) p_si, dh, eta, eps
...
real(dbl) set_ice_air_eq_at_t_p, t_si, p_si
real(dbl) a_si, dh, eta, eps
...
real(dbl) ice_air_iteration_at_t_p, t_si, p_si, eps, a_si, d_air_si
...
real(dbl) ice_air_iteration_at_a_eta, a_si, eta_si, eps, t_si, p_si, d_air_si
...
real(dbl) ice_air_iteration_at_a_t, a_si, t_si, eps, p_si, d_air_si
real(dbl) fh, fh_a, fh_d
real(dbl) fh_ad, fh_dd
real(dbl) gi, gi_p
real(dbl) dh, p
...
real(dbl) ice_air_iteration_at_a_p, a_si, p_si, eps, t_si, d_air_si
