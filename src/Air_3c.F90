...
character*11, private :: version = '11 May 2017'
...
real(dbl) ctrl_init_pottemp, ctrl_eps_exit_pottemp
...
!CHECK VALUES WITH DEFAULT SETTINGS, V1.2 (11th May 2017):
!air_h_si( 0, 0, 0, 0.9, 900, 1E5) = 274592.611783
!air_h_si( 1, 0, 0, 0.9, 900, 1E5) =-229706.905765
!air_h_si( 0, 1, 0, 0.9, 900, 1E5) = 297.403043058
!air_h_si( 0, 0, 1, 0.9, 900, 1E5) = 0.903262695636
!air_h_si( 2, 0, 0, 0.9, 900, 1E5) = 13963273.0104
!air_h_si( 1, 1, 0, 0.9, 900, 1E5) = 1676.85098552
!air_h_si( 1, 0, 1, 0.9, 900, 1E5) = 4.89772680822
!air_h_si( 0, 2, 0, 0.9, 900, 1E5) = 0.223684689765
!air_h_si( 0, 1, 1, 0.9, 900, 1E5) = 7.15703143992E-04
!air_h_si( 0, 0, 2, 0.9, 900, 1E5) =-6.78105152859E-06
...
real(dbl) air_h_si, a_si, eta_si, p_si
real(dbl) t
...
real(dbl) air_potdensity_si, a_si, t_si, p_si, pr_si
real(dbl) s, v
...
real(dbl) air_potenthalpy_si, a_si, t_si, p_si, pr_si
real(dbl) s
...
real(dbl) air_pottemp_si, a_si, t_si, p_si, pr_si
real(dbl) s
...
real(dbl) air_temperature_si, a_si, eta_si, p_si
real(dbl) t, eps
integer maxit
...
real(dbl) air_a_eta_p_derivatives_si, a_si, t_si, p_si
      ...
      case(0)
        select case(drv_p)
          ...
          case(1)
            ...
            if(g_tt == errorreturn) return
            if(g_tt == 0d0) return
			g_tp = air_g_si(0, 1, 1, a_si, t_si, p_si)     !added SIA v3_01_3 11th May 2017
            if(g_tp == errorreturn) return
...
real(dbl) pottemp_iteration, a_si, eta_si, p_si, t_si, eps
...
real(dbl) aux_temperature_si, a_si, eta_si, p_si
real(dbl) tt, pt, ra, rw, cpa, cpv
