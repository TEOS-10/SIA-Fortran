...
real(dbl), private :: ctrl_init_s_sea, ctrl_init_t, ctrl_init_d_liq, &
        ctrl_eps_exit_s, ctrl_eps_exit_t, ctrl_eps_exit_p
real(dbl), private :: equi_sea_ice_s, equi_sea_ice_t, equi_sea_ice_p, &
        equi_sea_ice_d_liq
...
implicit none
real(dbl) sea_ice_brinesalinity_si, t_si, p_si
...
implicit none
real(dbl) sea_ice_brinefraction_seaice_si, ssi_si, t_si, p_si
real(dbl) s
...
implicit none
real(dbl) sea_ice_meltingpressure_si, sa_si, t_si
...
implicit none
real(dbl) sea_ice_freezingtemperature_si, sa_si, p_si
...
real(dbl) sea_ice_dtfdp_si, sa_si, p_si
real(dbl) tf, s_t, s_p
...
real(dbl) sea_ice_dtfds_si, sa_si, p_si
real(dbl) tf, s_t
...
real(dbl) sea_ice_g_si, s_si, t_si, p_si
real(dbl) g, s, w, d
real(dbl) gb, gi
real(dbl) gb_p, gi_p
real(dbl) gb_t, gi_t
real(dbl) gb_pp, gi_pp, gm_pp
real(dbl) gb_tp, gi_tp, gm_tp
real(dbl) gb_tt, gi_tt, gm_tt
...
implicit none
real(dbl) sea_ice_cp_seaice_si, ssi_si, t_si, p_si
real(dbl) g_tt
...
implicit none
real(dbl) sea_ice_density_seaice_si, ssi_si, t_si, p_si
real(dbl) g_p
...
implicit none
real(dbl) sea_ice_enthalpy_seaice_si, ssi_si, t_si, p_si
real(dbl) g, g_t
...
implicit none
real(dbl) sea_ice_entropy_seaice_si, ssi_si, t_si, p_si
real(dbl) g_t
...
implicit none
real(dbl) sea_ice_expansion_seaice_si, ssi_si, t_si, p_si
real(dbl) g_p, g_tp
...
implicit none
real(dbl) sea_ice_kappa_t_seaice_si, t_si, p_si, ssi_si
real(dbl) g_p, g_pp
...
implicit none
integer drv_t, drv_p
real(dbl) sea_ice_s_si, t_si, p_si
real(dbl) d  !DILUTION COEFF
real(dbl) s  !BRINE SALINITY

real(dbl) gb_p, gi_p, gb_sp
real(dbl) gb_t, gi_t, gb_st
...
implicit none
integer maxit
real(dbl) set_sea_ice_eq_at_t_p, t_si, p_si
real(dbl) s_norm
real(dbl) d
real(dbl) s, eps
...
implicit none
integer maxit
real(dbl) set_sea_ice_eq_at_s_p, sa_si, p_si
real(dbl) t, d, eps
real(dbl) tt, dt
...
implicit none
integer maxit
real(dbl) set_sea_ice_eq_at_s_t, sa_si, t_si
real(dbl) eps
real(dbl) d_si, p_si
real(dbl) dt
...
implicit none
real(dbl) sea_ice_density_ice_si
...
implicit none
real(dbl) sea_ice_density_sea_si
...
implicit none
real(dbl) sea_ice_enthalpy_melt_si
real(dbl) s, t, p, d
...
implicit none
real(dbl) sea_ice_volume_melt_si
real(dbl) s, t, p, d
...
implicit none
real(dbl) sea_ice_enthalpy_ice_si
...
implicit none
real(dbl) sea_ice_enthalpy_sea_si
...
implicit none
real(dbl) sea_ice_entropy_ice_si 
...
implicit none
real(dbl) sea_ice_entropy_sea_si
...
implicit none
real(dbl) sea_ice_temperature_si
...
implicit none
integer it, maxit
real(dbl) sea_ice_iteration_at_t_p, t_si, p_si, eps, s_sea_si
real(dbl) s, ds
real(dbl) mus, sgss
real(dbl) gl, gi
...
implicit none
integer it, maxit
real(dbl) sea_ice_iteration_at_s_t, sa_si, t_si, eps, d_si, p_si
real(dbl) p, d, dd
real(dbl) mus, p_old
real(dbl) gl, gi, gi_p
real(dbl) gs_p, gs_sp
...
implicit none
integer it, maxit
real(dbl) sea_ice_iteration_at_s_p, sa_si, p_si, eps, d_si, t_si
real(dbl) d, dd, t, dt
real(dbl) mus, p
real(dbl) gl, gi, gi_t
real(dbl) gs_t, gs_st
...
implicit none
real(dbl) aux_freezingtemperature_si, sa_si, p_si
real(dbl) tt, pt, alpha, chi
...
implicit none
real(dbl) aux_liq_density_correlation_si, t_si
real(dbl) tt, dt, a1, a2, a3
real(dbl) tr, dr
...
implicit none
real(dbl) aux_meltingpressure_si, sa_si, t_si
real(dbl) tt, pt, alpha, chi
