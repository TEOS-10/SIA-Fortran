...
real(dbl) sea_g_si, sa_si, t_si, p_si
real(dbl) gw, gs
...
implicit none
real(dbl) sea_chempot_h2o_si, sa_si, t_si, p_si
real(dbl) g, mu_s
...
implicit none
real(dbl) sea_chempot_rel_si, sa_si, t_si, p_si
...
implicit none
real(dbl) sea_g_contraction_t_si, sa_si, t_si, p_si
real(dbl) g_p, gsp
...
implicit none
real(dbl) sea_cp_si, sa_si, t_si, p_si
real(dbl) g_tt
...
implicit none
real(dbl) sea_density_si, sa_si, t_si, p_si
real(dbl) g_p
...
implicit none
real(dbl) sea_enthalpy_si, sa_si, t_si, p_si
real(dbl) g, g_t
...
implicit none
real(dbl) sea_entropy_si, sa_si, t_si, p_si
real(dbl) g_t
...
implicit none
real(dbl) sea_g_expansion_t_si, sa_si, t_si, p_si
real(dbl) g_tp, g_p
...
implicit none
real(dbl) sea_gibbs_energy_si, sa_si, t_si, p_si
...
implicit none
real(dbl) sea_internal_energy_si, sa_si, t_si, p_si
real(dbl) g, g_t, g_p
...
implicit none
real(dbl) sea_kappa_s_si, sa_si, t_si, p_si
real(dbl) g_p
real(dbl) g_tp, g_tt, g_pp
...
implicit none
real(dbl) sea_kappa_t_si, sa_si, t_si, p_si
real(dbl) g_p, g_pp
...
implicit none
real(dbl) sea_lapserate_si, sa_si, t_si, p_si
real(dbl) g_tp, g_tt
...
implicit none
real(dbl) sea_osm_coeff_si, sa_si, t_si, p_si
...
implicit none
real(dbl) sea_soundspeed_si, sa_si, t_si, p_si
real(dbl) g_p, c
real(dbl) g_tp, g_tt, g_pp
...
!OUTPUT:
!T_MD(S,P) TEMPERATURE OF MAXIMUM DENSITY IN K, BY NEWTON ITERATION OF D2G/DTDP = 0
...
real(dbl) sea_temp_maxdensity_si, sa_si, p_si
real(dbl) g_tp, g_ttp, t, dt
real(dbl) h, t0, eps
integer iter, maxit
...
integer icol
real(dbl) s, t, p, rho
real(dbl) zg(3), zdgds(3), zdgdt(3), zdgdp(3), zd2gdsdp(3), zd2gdt2(3)
real(dbl) zd2gdtdp(3), zd2gdp2(3), zh(3), zf(3), zu(3), zsigma(3)
real(dbl) zrho(3), zcp(3), zc(3), zmuw(3)
real(dbl) g, gs, gt, gp, gss, gtt, gpp, gst, gsp, gtp, q
